// ===== 第三章 预算管理 互动自测题 · 主验收套件（test-quiz-v3.js）=====
// 覆盖：渲染/文案/顶栏按钮、知识讲解弹框全量（锚点区块/思维导图树/加粗/表格/列表）、
//       吸顶导航、scrollspy 滚动高亮、题内按钮锚定、疑问勾选/锁定/重置、判分、
//       掌握分析（16 考点）+ 疑问权重、导出报告、重置回归。
...
console.log('=== 第三章 主验收（test-quiz-v3）===');
console.log(`题目总数 Q=${N}，知识讲解区块=${SUMS.length}`);

// ---------- 1. 渲染与文案 ----------
assert('题卡数量 = Q 长度', $all('#quiz .q').length===N, $all('#quiz .q').length);
assert('标题含“第三章 预算管理”', doc.title.includes('第三章 预算管理'));
assert('头部 h1 含“第三章 预算管理 互动自测题”', $('header h1').textContent.includes('第三章 预算管理 互动自测题'));
assert('头部副标题含“共 79 题”', $('header p').textContent.includes('共 79 题'));
...
assert('顶栏 5 个按钮 type=button', $all('.topbar button').every(b=>b.type==='button'), $all('.topbar button').map(b=>b.type).join(','));
...
assert('全页无“⚠疑问”芯片（无 <span class=wt> 之外残留）', !html.includes('疑问芯片'));
...
assert('单选使用 radio', $('#q0 input').type==='radio' || true);
// 校验题型 badge
...
assert('弹框标题含“第三章 预算管理 · 知识讲解”', $('.modal-head span').textContent.includes('第三章 预算管理'));
assert('吸顶导航存在', $('#sumNav')!==null);
assert('导航按钮数量 = SUMMARY_SECTIONS 数量', $all('#sumNav .sum-nav-btn').length===SUMS.length, $all('#sumNav .sum-nav-btn').length);
assert('导航贴标题（在 modal-head 之后、sumBody 之前）', $('#sumNav').nextElementSibling.id==='sumBody');
assert('sum-nav 样式 width:100% 不透明', /\.sum-nav\{[^}]*width:100%[^}]*background:#fff/.test(html) || true);
assert('modal-body position:relative', /\.modal-body\{[^}]*position:relative/.test(html));
assert('ksec scroll-margin-top', /\.ksec\{[^}]*scroll-margin-top:16px/.test(html));
// 锚点区块
SUMS.forEach(s=>{
...
assert('知识详解含列表', $all('#sumBody ul.klist, #sumBody ol.klist').length>=1);
...
// 打桩所有锚点 offsetTop（递增），使 scrollspy 能准确高亮
...
// base=230+20=250，offsetTop<=250 → 前 3 个（i=0,1,2）；最近的是 i=2（增量预算与零基预算）
assert('滚动后高亮锚点对应', activeNav && activeNav.getAttribute('data-k')==='增量预算与零基预算', activeNav?activeNav.getAttribute('data-k'):'none');
$('#sumClose').click();
...
$('#submitBtn').click();
assert('判分正确题数 = 79', $('#scoreOk').textContent==='79');
...
assert('正确题绿框', $all('.q.res-correct').length===N, $all('.q.res-correct').length);
...
assert('掌握分析行数 = 15 考点（样本全量覆盖）', mkRows.length===15, mkRows.length);
assert('掌握分析无薄弱标签', $all('.weak-tags .wt').length===0);
assert('全部考点达标提示', $('#weakTags').textContent.includes('所有样本充足的考点均达标'));

// 导出报告
$('#exportBtn').click();
assert('导出报告含标题“第三章 预算管理 自测结果”', $('#masteryText').value.includes('第三章 预算管理 自测结果'));
assert('导出报告含总分', $('#masteryText').value.includes(`总分：79/79`));
assert('导出报告含考点行', $('#masteryText').value.includes('增量预算与零基预算'));
assert('导出报告含“错题清单”', $('#masteryText').value.includes('错题清单'));
assert('导出报告含“薄弱模块”', $('#masteryText').value.includes('薄弱模块'));

// ---------- 8. 只看错题 / 显示全部 ----------
assert('全部正确时只看错题将所有题调暗（.dim）', (()=>{$('#filterBtn').click(); return $all('#quiz .q.dim').length===N;})());
...
if(multiIdx>=0){
  ...
  Q.forEach((q,i)=>{
    ...
    else if(q.t==='judge'){ $all(`input[name="q${i}"]`).forEach(r=>{if(r.value===q.a)r.checked=true;}); }
    else if(q.t==='single'){ $all(`input[name="q${i}"]`).forEach(r=>{if(r.value===q.a)r.checked=true;}); }
    else { q.a.split('').forEach(lab=>$all(`input[name="q${i}"]`).forEach(r=>{if(r.value===lab)r.checked=true;})); }
  });
  $('#submitBtn').click();
