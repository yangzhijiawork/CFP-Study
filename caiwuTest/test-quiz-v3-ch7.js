...
const fs = require('fs');
const html = fs.readFileSync('hudongxiti/第七章营运资金管理互动习题.html','utf-8');
...
console.log('=== 第七章 主验收（test-quiz-v3-ch7）===');
console.log(`题目总数 Q=${N}，知识讲解区块=${SUMS.length}`);

// ---------- 1. 渲染与文案 ----------
assert('题卡数量 = Q 长度', $all('#quiz .q').length===N, $all('#quiz .q').length);
assert('标题含“第七章 营运资金管理”', doc.title.includes('第七章 营运资金管理'));
assert('头部 h1 含“第七章 营运资金管理”', $('header h1').textContent.includes('第七章 营运资金管理'));
...
assert('弹框标题含“第七章 营运资金管理”', $('.modal-head span').textContent.includes('第七章 营运资金管理'));
...
assert('滚动后高亮锚点对应', activeNav && activeNav.getAttribute('data-k')==='现金管理', activeNav?activeNav.getAttribute('data-k'):'none');
$('#sumClose').click();
...
// 导出报告
$('#exportBtn').click();
assert('导出报告含标题“第七章 营运资金管理 自测结果”', $('#masteryText').value.includes('第七章 营运资金管理 自测结果'));
assert('导出报告含总分', $('#masteryText').value.includes(`总分：${N}/${N}`));
assert('导出报告含考点行', $('#masteryText').value.includes('营运资金管理概述'));
