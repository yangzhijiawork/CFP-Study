// ===== 第四章 筹资管理（上） 互动自测题 · 主验收套件（test-quiz-v3-ch4.js）=====
const { JSDOM } = require('jsdom');
const fs = require('fs');
const html = fs.readFileSync('hudongxiti/第四章筹资管理上互动习题.html','utf-8');
...
const N = Q.length;
const NUM_KPOINTS = 14; // 第四章有 14 个考点

console.log('=== 第四章 主验收（test-quiz-v3-ch4）===');
console.log(`题目总数 Q=${N}，知识讲解区块=${SUMS.length}`);

// ---------- 1. 渲染与文案 ----------
assert('题卡数量 = Q 长度', $all('#quiz .q').length===N, $all('#quiz .q').length);
assert('标题含“第四章 筹资管理”', doc.title.includes('第四章 筹资管理'));
assert('头部 h1 含“第四章 筹资管理（上） 互动自测题”', $('header h1').textContent.includes('第四章 筹资管理（上） 互动自测题'));
...
assert('全页无“⚠疑问”芯片', !html.includes('疑问芯片'));
...
assert('弹框标题含“第四章 筹资管理（上） · 知识讲解”', $('.modal-head span').textContent.includes('第四章 筹资管理（上）'));
...
assert('滚动后高亮锚点对应', activeNav && activeNav.getAttribute('data-k')==='直接vs间接筹资', activeNav?activeNav.getAttribute('data-k'):'none');
$('#sumClose').click();
...
// 导出报告
$('#exportBtn').click();
assert('导出报告含标题“第四章 筹资管理（上） 自测结果”', $('#masteryText').value.includes('第四章 筹资管理（上） 自测结果'));
assert('导出报告含总分', $('#masteryText').value.includes(`总分：${N}/${N}`));
assert('导出报告含考点行', $('#masteryText').value.includes('筹资动机'));
