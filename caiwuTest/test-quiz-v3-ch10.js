...
const fs = require('fs');
const html = fs.readFileSync('hudongxiti/第十章财务分析与评价互动习题.html','utf-8');
...
console.log('=== 第十章 主验收（test-quiz-v3-ch10）===');
console.log(`题目总数 Q=${N}，知识讲解区块=${SUMS.length}`);

// ---------- 1. 渲染与文案 ----------
assert('题卡数量 = Q 长度', $all('#quiz .q').length===N, $all('#quiz .q').length);
assert('标题含“第十章 财务分析与评价”', doc.title.includes('第十章 财务分析与评价'));
assert('头部 h1 含“第十章 财务分析与评价”', $('header h1').textContent.includes('第十章 财务分析与评价'));
...
assert('弹框标题含“第十章 财务分析与评价”', $('.modal-head span').textContent.includes('第十章 财务分析与评价'));
...
assert('滚动后高亮锚点对应', activeNav && activeNav.getAttribute('data-k')==='偿债能力分析', activeNav?activeNav.getAttribute('data-k'):'none');
$('#sumClose').click();
...
// 导出报告
$('#exportBtn').click();
assert('导出报告含标题“第十章 财务分析与评价 自测结果”', $('#masteryText').value.includes('第十章 财务分析与评价 自测结果'));
assert('导出报告含总分', $('#masteryText').value.includes(`总分：${N}/${N}`));
assert('导出报告含考点行', $('#masteryText').value.includes('财务分析方法'));
