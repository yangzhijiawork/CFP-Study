// ===== 第四章 筹资管理（上） 互动自测题 · 深度验收套件（test-quiz-deep-ch4.js）=====
...
console.log('=== 第四章 深度验收（test-quiz-deep-ch4）===');
...
assert('全部标记后疑问计数 = '+N, $('#doubtcnt').textContent===String(N));
...
assert('提交后疑问计数保持 '+N, $('#doubtcnt').textContent===String(N));
$('#resetBtn').click();
...
assert('疑问汇总涉及考点列出', $('#doubtSummary').textContent.includes('筹资动机'));
...
$('#filterBtn').click();
assert('只看错题后显示错题数（非 .dim）', $all('#quiz .q:not(.dim)').length===wrongCount);
...
// ---------- 10. 锚点导航点击 ----------
$('#chapterBtn').click();
const targetNav = $('#sumNav .sum-nav-btn[data-k="可转换债券"]');
targetNav.click();
assert('点击导航按钮不报错且存在目标锚点', $('#kp-可转换债券')!==null);
$('#sumClose').click();
