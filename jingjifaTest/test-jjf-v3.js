...
const kw = ['整章思维导图', '民事法律行为', '代理', '仲裁', '民事诉讼', '行政复议', '行政诉讼', '真题印证', '知识链闭环', '全三单元完整详解', '表见代理', '无效'];
kw.forEach(k => assert(sumTxt.includes(k), `弹框包含原文关键内容【${k}】`));
ok('弹框含原文全部关键内容（考情/思维导图/民事法律行为/代理/三途径/真题印证/闭环）');
...
// 题内知识讲解按钮 → 弹框 + 定位
click(qsel('.btn-know')[0]); // 第1题 k=民事行为能力
assert(q1('#sumMask').classList.contains('show'), '题内知识讲解按钮打开弹框');
assert(!!q1('#kp-民事行为能力'), '目标考点区块 #kp-民事行为能力 存在');
ok('题内按钮锚定到对应考点区块');

// scrollspy：14 个 section 的 offsetTop 打桩
const secKs = ['思维导图','民事行为能力','民事法律行为概述','民事法律行为效力','附条件与附期限','代理制度概述','特殊代理行为','代理关系终止','民事诉讼','行政复议','行政诉讼','仲裁','速查表','闭环'];
const tops = {};
...
scrollTo(1200); // 民事法律行为概述(1000)已过、效力(1500)未到
let act = q1('.sum-nav-btn.active');
assert(act && act.getAttribute('data-k') === '民事法律行为概述', `滚动高亮【民事法律行为概述】(实际 ${act && act.getAttribute('data-k')})`);
scrollTo(3200); // 特殊代理行为(3000)已过、代理关系终止(3500)未到
act = q1('.sum-nav-btn.active');
assert(act && act.getAttribute('data-k') === '特殊代理行为', `滚动高亮【特殊代理行为】(实际 ${act && act.getAttribute('data-k')})`);
scrollTo(6200); // 速查表(6000)已过、闭环(6500)未到
act = q1('.sum-nav-btn.active');
